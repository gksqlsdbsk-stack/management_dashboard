...
> **Phase 5(입력 현황·연간 목표)까지 구현되었습니다.** 로그인·로그아웃, 관리자의 사용자·부서·입력 항목 관리, 입력 현황 조회·연간 목표 설정, 직원의 월 실적 입력(임시 저장·제출·엑셀/CSV 업로드)이 동작합니다. 지표 계산과 대시보드·리포트는 이후 Phase에서 구현됩니다.
...
- `연간 목표`에서 연도별로 매출액·영업이익률·가동률·생산량·신규 수주액 목표를 입력합니다. 값을 비우고 저장하면 목표가 삭제됩니다(목표 미설정). 목표 대비 달성률은 Phase 6의 대시보드에서 표시됩니다.
